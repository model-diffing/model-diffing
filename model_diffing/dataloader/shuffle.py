...
def batch_shuffle_tensor_iterator_BX(
    ...
    logger.info(f"shuffle buffer ratio is {shuffle_buffer_size / yield_batch_size:.2f}")

    first_tensor_X = next(tensor_iterator_X)
    ...
    def sample_BX():
        batch_indices = random.sample(list(ready_indices), yield_batch_size)
        ready_indices.difference_update(batch_indices)
