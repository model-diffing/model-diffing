...
COLORS = {'normal': '#8E8E99', # off black
            'bottom_x': '#0081A7', # teal
            'bottom_y': '#009B72', # green
            'bottom_both': '#D66853', # reddish
           }

def plot_ft_rotations(x_vals: np.ndarray[Any, np.dtype[np.float64]], 
                      y_vals: np.ndarray[Any, np.dtype[np.float64]], 
                      x_label: str = 'Data1', y_label: str = 'Data2',
                      title: str = 'Pretty Plot', 
                      hover_text: dict[int, str]= {}, 
    ...
    Args:
        ...
        hover_text: A dictionary mapping feature indices to hover text (optional, and can contain a subset of features).
    ...
    for cat_name, (cat_indices, color) in categories.items():
        ...
        if hover_pts:
            hover_source = bk.ColumnDataSource(data={
                'x': [x_vals[i] for i in hover_pts],
                'y': [y_vals[i] for i in hover_pts],
                'hover_text': [hover_text[i] for i in hover_pts]
            })
            # Add hover points with solid outline
            hover_renderer = p.scatter('x', 'y', source=hover_source, 
                                     size=scatter_size,
                                     fill_color=color,
                                     fill_alpha=1,
                                     line_color=None,
